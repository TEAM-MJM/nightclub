    ...
    <section>
      ...
      <form action={postComment} className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="yourName" placeholder="Your Name" defaultValue={!state.success ? state.yourName : ""} className="border w-full py-6 px-3"></input>
        </div>
        <div>
          {state.error?.yourEmail && <p className="text-red-500">{state.error.yourEmail}</p>}
          <input type="email" name="yourEmail" placeholder="Your Email" defaultValue={!state.success ? state.yourEmail : ""} className="border px-3 w-full py-6"></input>
        </div>
